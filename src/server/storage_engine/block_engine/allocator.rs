...
impl BlockGroup {
    ...
    fn allocator_slice_space(&mut self, mut chunk_size: u64) -> Vec<AllocatorEntry> {
        let mut result_vec = Vec::new();
    ...
    fn release_space(&mut self, len: u64, begin: u64) -> bool {
        let mut chunk_size = len / CHUNK;
        if len - chunk_size * CHUNK > 0 {
            chunk_size += 1;
        }
