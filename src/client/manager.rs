...
pub struct Client {
    ...
    pub parent_inodes: DashMap<u64, u64>,
}
...
impl Client {
    ...
    pub fn getattr_remote(&self, ino: u64, reply: ReplyAttr) {
        ...
        if self.inodes_reverse.contains_key(&ino) {
            let path = self.inodes_reverse.get(&ino).unwrap().clone();
            let server_address = self.get_connection_address(&path).unwrap();
            let mut status = 0i32;
            let mut rsp_flags = 0u32;
        ...
        if self.inodes_reverse.contains_key(&ino) {
            ...
            match result {
                Ok(_) => {
                    ...
                    //reply.add(0, 0, entry.kind, entry.name);
